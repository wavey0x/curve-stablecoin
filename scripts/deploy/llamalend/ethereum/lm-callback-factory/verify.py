#!/usr/bin/env python3
"""Verify the LMCallback blueprint + LMCallbackFactory on Etherscan (Mainnet).

Companion to deploy.py: the addresses and the factory's
constructor args are read from the report that script writes.
...
Run:
    ETHERSCAN_API_KEY=... python scripts/deploy/llamalend/ethereum/lm-callback-factory/\
verify.py
"""
...
# ---------------------------------------------------------------------------
# Contract sources (must match deploy.py)
# ---------------------------------------------------------------------------

LM_CALLBACK_SRC = "curve_stablecoin/lm_callback/LMCallback.vy"
LM_CALLBACK_NAME = f"{LM_CALLBACK_SRC}:LMCallback"
LM_CALLBACK_FACTORY_SRC = "curve_stablecoin/lm_callback/LMCallbackFactory.vy"
LM_CALLBACK_FACTORY_NAME = f"{LM_CALLBACK_FACTORY_SRC}:LMCallbackFactory"

DEFAULT_DEPLOYMENT = "deployments/llamalend/ethereum/lm-callback-factory.jsonc"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Verify the LMCallback blueprint + LMCallbackFactory on Etherscan"
    )
    parser.add_argument(
        "--deployment",
        default=DEFAULT_DEPLOYMENT,
        help="Path to the deployment report written by deploy.py",
    )
    ...
    contracts = [
        ...
        (
            factory_addr,
            "LMCallbackFactory (Vyper 0.4.3)",
            LM_CALLBACK_FACTORY_NAME,
